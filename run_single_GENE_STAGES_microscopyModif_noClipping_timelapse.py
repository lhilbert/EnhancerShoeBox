...
def makeFakeMicroscopyImages(
    ...
    """
    Append one time frame to an in-memory OME image buffer.

    Output axes: (T, C, Z, Y, X)
    """
    ...
    # -----------------------------
    VOXEL_SIZE = 0.25
    PSF_SIGMA_PHYSICAL = 0.4
    PHOTONS_PER_ATOM = 100.0
    DETECTOR_MAX = 65535

    sigma_pixels = PSF_SIGMA_PHYSICAL / VOXEL_SIZE

    # High-resolution voxel grid
    x_edges = np.arange(-11, 11 + VOXEL_SIZE, VOXEL_SIZE)
    y_edges = np.arange(-9,  9 + VOXEL_SIZE, VOXEL_SIZE)
    z_edges = np.arange(-7,  7 + VOXEL_SIZE, VOXEL_SIZE)

    def make_channel(atom_type_filter):
        atoms = np.where(np.isin(atomTypes.flatten(), atom_type_filter))[0]
        ...
        H = np.transpose(H, (2, 1, 0)).astype(np.float32)
        H = gaussian_filter(H, sigma=sigma_pixels)

        H *= PHOTONS_PER_ATOM
        np.clip(H, 0, DETECTOR_MAX, out=H)

        return H.astype(np.uint16)

    # Channels (now INCLUDING super-enhancer)
    frame = np.stack([
        make_channel([8]),        # Ser5P
        make_channel([7, 11]),    # Ser2P
        make_channel([9]),        # RegulatoryChromatin (SUPER-ENHANCER)
        make_channel([1, 2, 6, 7, 9, 10, 11]),  # Chromatin
        make_channel([2, 6, 7, 10, 11]),        # Gene
        make_channel([3])         # Actin
    ], axis=0)
...
for i in range(NRuns+treatment_duration):
    ...
    # ============================================================
    # Synthetic microscopy images (TIME-LAPSE MODE)
    # ============================================================

    # Microscopy buffer initialized once (before the simulation loop)
    if i == 0 and make_microscopy:
        microscopy_buffer = []

    # Gather atom positions and types
    atomPositions = np.reshape(
        np.ctypeslib.as_array(
            Group.lmp.gather_atoms('x', 1, 3)
        ),
        (Group.L.system.natoms, 3)
    )

    atomTypes = np.reshape(
        np.ctypeslib.as_array(
            Group.lmp.gather_atoms('type', 0, 1)
        ),
        (Group.L.system.natoms, 1)
    )

    # Append one microscopy frame every 10 steps
    if (i + 1) % 10 == 0 and make_microscopy:
        makeFakeMicroscopyImages(
            microscopy_buffer,
            atomPositions,
            atomTypes,
            time_index=i + 1
    ...
    # ------------------------------------------------------------
    # WRITE FINAL OME-TIFF ONCE (after last simulation step)
    # ------------------------------------------------------------
    if i == NRuns + treatment_duration - 1 and make_microscopy:
        ...
        final_stack = np.stack(microscopy_buffer, axis=0)

        tifffile.imwrite(
            os.path.join(out_dir, "synthetic_microscopy_timelapse.ome.tif"),
            final_stack,
    ...
    ser5p_around_cluster[-1] = diff_list       
