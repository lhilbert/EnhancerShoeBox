...
def makeFakeMicroscopyImages(
    ...
    """
    Generate multi-channel 3D synthetic microscopy image (OME-TIFF)

    sigma is optional and ignored (kept for backward compatibility)
    Axis order: (C, Z, Y, X)
    """
    ...
    # -----------------------------
    VOXEL_SIZE = 0.25          # simulation units
    PSF_SIGMA_PHYSICAL = 0.4   # same units
    PHOTONS_PER_ATOM = 100.0
    DETECTOR_MAX = 65535       # 16-bit
    ...
    def make_channel(atom_type_filter):
        ...
        # Photon scaling + detector clipping
        H *= PHOTONS_PER_ATOM
    ...
    tifffile.imwrite(
        ...
        metadata={
            "axes": "CZYX",
            "Channel": {
                "Name": ["Ser5P", "Ser2P", "RegulatoryChromatin", "Chromatin", "Gene", "Actin"]
            },
            "PhysicalSizeX": VOXEL_SIZE,
            "PhysicalSizeY": VOXEL_SIZE,
            "PhysicalSizeZ": VOXEL_SIZE
        }
    )
