...
try:
    opts, args = getopt.getopt(sys.argv[1:],"hb:r:t:o:m:c:x:p:a:x:z:",["box=","repeat=","total=","outfolder=","make_images=","condition=","promoter=","activation=","threshold=","actin="])
except getopt.GetoptError:
    print('run_single_ACTIN_SMALL_BOX.py -b <box_size> -r <repeat> -t <total_runs> -o <out_folder> -m <make_images> -c <condition> -p <promoter_length> -a <activation_rate> -x <threshold> -z <actin>')
    sys.exit(2)
for opt, arg in opts:
    if opt == '-h':
        print('run_single_ACTIN_SMALL_BOX.py -b <box_size> -r <repeat> -t <total_runs> -o <out_folder> -m <make_images> -c <condition> -p <promoter_length> -a <activation_rate> -x <threshold> -z <actin>')
...
folders_to_make = ["figures", "dump_files", "image_files", "microscopy_files", "summaries"]
...
fileBase_input='ser5P'+str(number_of_Ser5P)+'_actin'+str(init_free_actin)+'_RBP'+str(number_of_RBP)+'_0RNP_promoter'+str(promoter_length)+'_bs'+str(box)
...
if condition=="LatB" or condition=="SwinA_nopol":
    t_polymerization_on = 2*NRuns # 2*NRuns # 25 / 40 # 1 min = 60s
else:
    t_polymerization_on = 40 # 40
    
t_polymerization_off = 2*NRuns

t_ser5p_on = 15 # polymerization can't begin before Ser5P on
t_induction_on=150 # 3 min = 180s
if condition=="Flavopiridol":
    t_activation_on=2*NRuns
else:
    t_activation_on=t_induction_on # 5 min = 300s
    
t_transcription_on=2*NRuns # 5 min = 300s
t_induction_off=2*NRuns # 5 min = 300s
t_activation_off=2*NRuns # 10 min = 600s
t_transcription_off=2*NRuns # 10 min = 600s

tImageDump=10*tRun

if gene_switching_off:
    t_transcription_on=2*NRuns # 5 min = 300s
    t_induction_on=2*NRuns # 2 min = 120s
    t_activation_on=2*NRuns # 5 min = 300s
# each run is 6s
...
t_off_plus=2*NRuns # (k_on_plus/k_off_plus) 8
t_off_minus=5 # (k_on_plus/k_off_minus)
pon=0.3 # 0.3
p_nucleation=0.99999
...
while 1:
    ...
    # Group.L.fix('wallhi all wall/harmonic xlo EDGE 50 0.0 3.0 xhi EDGE 50 0.0 3.0 ylo EDGE 50 0.0 3.0 yhi EDGE 50 0.0 3.0 zlo EDGE 50 0.0 3.0 zhi EDGE 50 0.0 3.0')
    
    Group.L.region('centerSlice block',-50,50,-50,50,-3,3)
    Group.L.group('atomsCS dynamic all region centerSlice')
    Group.L.group('actinCS dynamic Actin region centerSlice')
    ...
    if make_snapshots:
        ...
        # image_dump_string = 'imageDumpActin pad 7 backcolor white adiam 1*2 0.5 adiam 3*5 0.4 adiam 6*7 0.5 adiam 8 0.5 adiam 9*10 0.5 bdiam 1 0.05 bdiam 2 0.2 color verydarkgrey 0.2 0.2 0.2 acolor 1 lightgrey acolor 2 verydarkgrey acolor 3 chartreuse acolor 4 yellow acolor 5 aqua acolor 6 maroon acolor 7 darkgreen acolor 8 indianred acolor 9 blue acolor 10 lightblue'
        # Group.L.dump('imageDumpActin Actin image',tImageDump, folder_name+'/image_files/ACTIN_*.ppm type type zoom',3,'size',1700,1000,'view',0,-90,'shiny 0.5 box no 1')
        # Group.L.dump_modify(image_dump_string)
        
        # image_dump_string = 'imageDumpSer5P pad 7 backcolor white adiam 1*2 0.5 adiam 3*5 0.4 adiam 6*7 0.5 adiam 8 0.5 adiam 9*10 0.5 bdiam 1 0.05 bdiam 2 0.2 color verydarkgrey 0.2 0.2 0.2 acolor 1 lightgrey acolor 2 verydarkgrey acolor 3 chartreuse acolor 4 yellow acolor 5 aqua acolor 6 maroon acolor 7 darkgreen acolor 8 indianred acolor 9 blue acolor 10 lightblue'
        # Group.L.dump('imageDumpSer5P Ser5P image',tImageDump, folder_name+'/image_files/Ser5P_*.ppm type type zoom',3,'size',1700,1000,'view',0,-90,'shiny 0.5 box no 1')
        # Group.L.dump_modify(image_dump_string)
    
    if dump_positions:
        # Group.L.dump('chromatinPositionDump InactiveChromatin custom 10000 dump_files/chromatinPositions.dump id mol type x y z')
        RBPRNP.add_dump('rbprnpPositionDump', 'custom 1000 '+out_folder+'/run'+str(run_number)+'/dump_files/rbprnpPositions_*.dump id mol type x y z', dump_modify_command=None)
        
    # Group.L.compute('chromatinBins InactiveChromatin chunk/atom bin/3d x 0.0 5.0 y 0.0 5.0 z 0.0 5.0')
    # Group.L.fix('chromatinBinsFix InactiveChromatin ave/chunk 100 10 1000 chromatinBins density/number ave one file '+folder_name+'/chromatin.bins')

    # Group.L.compute('actinBins Actin chunk/atom bin/3d x 0.0 5.0 y 0.0 5.0 z 0.0 5.0')
    # Group.L.fix('actinBinsFix Actin ave/chunk 100 10 1000 actinBins density/number ave one file '+folder_name+'/actin.bins')
    # Timestep and run an initial soft potential.
    # Do we need this if polymer is initialized such that adjacent monomer are placed nearby?
    Group.L.timestep(dt)
    break
...
for i in range(NRuns):
    ...
    if (i+1)>=t_polymerization_on and (i+1)<t_polymerization_off:
        # --------------------------------------------
        ...
        for j in range(len(free_actin)):
            # progress_bar(j, len(free_actin)-1, "Actin nucleation")
            # Monomer-monomer
        ...
        for j in range(len(filament_details)):
            # progress_bar(j, len(filament_details)-1, "Actin polymerization")
...
if make_plots:
    ...
    fig, ax = plt.subplots()
    plt.plot(delt*np.array(list(range(NRuns+1)))/60, n_rnp)
    ax.set_title('\# RNPs')
    ...
    for j in range(len(gene_start_atoms)):
        ...
        fig.savefig(out_folder+'/run'+str(run_number)+"/figures/gene"+str(j+1)+"_rg.pdf", bbox_inches="tight")
    ...
    for j in range(len(gene_start_atoms)):
        ...
        fig.savefig(out_folder+'/run'+str(run_number)+"/figures/gene"+str(j+1)+"_rp.pdf", bbox_inches="tight")
    
    for j in range(len(gene_start_atoms)):
        fig, ax = plt.subplots(2,1, sharex=True)
        ax[0].plot([delt*x/60 for x in list(range(len(d_rg)))], [d_rg[x][j]*sig_chromatin for x in range(len(d_rg))], color=(0.5,0.5,0.5), linewidth=0.5)
        ax[0].plot([delt*x/60 for x in active_runs[j]], [d_rg[x][j]*sig_chromatin for x in active_runs[j]], 'go', markersize=2)
        ax[0].plot([delt*x/60 for x in induced_runs[j]], [d_rg[x][j]*sig_chromatin for x in induced_runs[j]], 'ro', markersize=0.5)
        ax[1].plot([delt*x/60 for x in list(range(len(d_rg)))], [ser5p_around_gene[x][j] for x in range(len(d_rg))], color=(0.5,0.5,0.5), linewidth=0.5)
        ax[1].plot([delt*x/60 for x in active_runs[j]], [ser5p_around_gene[x][j] for x in active_runs[j]], 'go', markersize=2)
        ax[1].plot([delt*x/60 for x in induced_runs[j]], [ser5p_around_gene[x][j] for x in induced_runs[j]], 'ro', markersize=0.5)
        ax[1].plot([delt*x/60 for x in list(range(len(d_rg)))], [ser5p_to_activate for x in list(range(len(d_rg)))], color=(0.5,0.5,0.5), linewidth=0.5)
        ax[1].set_xlabel('Time (min)')
        ax[0].set_ylabel('Reg-TSS distance (nm)')
        ax[1].set_ylabel('No. of Ser5P around gene TSS')
        fig.savefig(out_folder+'/run'+str(run_number)+"/figures/gene_"+str(j+1)+"_rg_time.pdf", bbox_inches="tight")
        
    for j in range(len(gene_start_atoms)):
        fig, ax = plt.subplots(2,1, sharex=True)
        ax[0].plot([delt*x/60 for x in list(range(len(d_rp)))], [d_rp[x][j]*sig_chromatin for x in range(len(d_rp))], color=(0.5,0.5,0.5), linewidth=0.5)
        ax[0].plot([delt*x/60 for x in active_runs[j]], [d_rp[x][j]*sig_chromatin for x in active_runs[j]], 'go', markersize=2)
        ax[0].plot([delt*x/60 for x in induced_runs[j]], [d_rp[x][j]*sig_chromatin for x in induced_runs[j]], 'ro', markersize=0.5)
        ax[1].plot([delt*x/60 for x in list(range(len(d_rp)))], [ser5p_around_promoter[x][j] for x in range(len(d_rp))], color=(0.5,0.5,0.5), linewidth=0.5)
        ax[1].plot([delt*x/60 for x in active_runs[j]], [ser5p_around_promoter[x][j] for x in active_runs[j]], 'go', markersize=2)
        ax[1].plot([delt*x/60 for x in induced_runs[j]], [ser5p_around_promoter[x][j] for x in induced_runs[j]], 'ro', markersize=0.5)
        ax[1].plot([delt*x/60 for x in list(range(len(d_rp)))], [ser5p_to_activate for x in list(range(len(d_rp)))], color=(0.5,0.5,0.5), linewidth=0.5)
        ax[1].set_xlabel('Time (min)')
        ax[0].set_ylabel('Reg-Promoter distance (nm)')
        ax[1].set_ylabel('No. of Ser5P around promoter')
        fig.savefig(out_folder+'/run'+str(run_number)+"/figures/gene_"+str(j+1)+"_rp_time.pdf", bbox_inches="tight")
    ...
    for j in range(len(gene_start_atoms)): 
        ...
        plt.ylabel('Probability')
        fig.savefig(out_folder+'/run'+str(run_number)+"/figures/gene_"+str(j+1)+"_drg_distr.pdf", bbox_inches="tight")
    ...
    for j in range(len(gene_start_atoms)): 
        ...
        plt.ylabel('Probability')
        fig.savefig(out_folder+'/run'+str(run_number)+"/figures/gene_"+str(j+1)+"_ddrg_distr.pdf", bbox_inches="tight")
